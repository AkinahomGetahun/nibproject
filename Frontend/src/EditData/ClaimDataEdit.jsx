...
function ClaimDataEdit() {
  ...
  const getClaimsbyid = async () => {
    const response = await api.post(`/claims-by-id/${id}`);
    setFormData({
      branch: response.data.branch || " ",
      claimnumber: response.data.claimnumber || " ",
      policyclass: response.data.policyclass || " ",
      policynumber: response.data.policynumber || " ",
      totalclaimspaid: response.data.totalclaimspaid || " ",
      coinsurerrecovery: response.data.coinsurerrecovery || " ",
      facrecovery: response.data.facrecovery || " ",
      salvage: response.data.salvage || " ",
      totalrecovery: response.data.totalrecovery || "",
      insured: response.data.insured || " ",
      dateofloss: response.data.dateofloss || " ",
      treatyrecovery: response.data.treatyrecovery || "",
      notificationdate: response.data.notificationdate || " ",
      regdate: response.data.regdate || " ",
      dateclaimpaid: response.data.dateclaimpaid || "",
      descriptionofloss: response.data.descriptionofloss || " ",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await api.post("/edit-claims-data", formData);
      console.log(response);

      toast.success("Updated Claims Data Successfully!");
      setTimeout(() => {
        navigate("/claimtable");
      }, 2000);
    } catch (error) {
    ...
    <div className=" flex flex-col items-center justify-center  border-2 border-stone-200 lg:w-[1200px] mx-auto rounded-md bg-stone-50 ">
      <div className="">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col-2  gap-35 items-center justify-center ">
            ...
            <div className="w-70 flex flex-col gap-5 mt-4">
              <div className="flex flex-col gap-3 ">
                <label>Agency</label>
                <input
                  type="text"
          ...
          <div className="w-[100px] h-[30px] text-center text-stone-800  py-1 rounded-xl bg-[#fc973f] mt-9 mb-8 hover:bg-stone-900 hover:text-gray-200 duration-300">
            <ToastContainer
              position="top-center"
              autoClose={1500}
            ...
            />
            <button type="submit" className="font-semibold  py-1 ">
              Update
            </button>
          </div>
        </form>
